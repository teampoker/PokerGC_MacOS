...
#endif
	//html stuff
	QString createAccount(QString("<a href='http://teampoker.com/game/platform'>%1</a>").arg(tr("Create new user account")));
...
void internetGameLoginDialogImpl::regUserToggled(bool b)
{
	...
	if(b) {
		if(QString::fromUtf8(myConfig->readConfigString("MyRememberedNameDuringGuestLogin").c_str()).isEmpty()) {
			...
			//lineEdit_username->setText(QString(md5(lineEdit_password->text().toUtf8().constData()).c_str()));//remove
		} else {
...
void internetGameLoginDialogImpl::accept()
{
	...
	if(groupBox_reguser->isChecked()) {
		...
		//lineEdit_password->setText(QString("clave"));//remove
		//lineEdit_username->setText(lineEdit_password->text());//remove
		//lineEdit_password->setText(QString(md5(lineEdit_password->text().toUtf8().constData()).c_str()));//remove
		//lineEdit_password->setText(QString("5f4dcc3b5aa765d61d8327deb882cf99"));
		std::string str_temp = lineEdit_password->text().toUtf8().constData();
		//lineEdit_username->setText(QString(md5(str_temp).c_str()));//remove
		lineEdit_password->setText(QString(md5(str_temp).c_str()));//remove
		...
		guestName.sprintf("Alien%05d", guestId);
