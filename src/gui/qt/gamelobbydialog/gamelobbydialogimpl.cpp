...
//	setWindowFlags(Qt::Dialog | Qt::WindowMinimizeButtonHint);
	...
	//HTML stuff
	QString pokerthDotNet("<a href='http://teampoker.com/'>http://teampoker.com/</a>");
	QString clickToRanking(QString("<a href='http://platform.teampoker.com/'>%1</a>").arg(tr("Go to My Platform")));
	QString clickToSpectate(QString("<a href='http://teampoker.com/game/spectator'><b>%1</b></a>").arg(tr("Spectate")));
...
void gameLobbyDialogImpl::createGame()
{
	...
	if (myCreateInternetGameDialog->result() == QDialog::Accepted ) {
		...
		case GAME_TYPE_INVITE_ONLY: {
			...
			label_typeText->setText(tr("PRIVATE POKER TABLE(TM)"));
		}
		break;
...
void gameLobbyDialogImpl::showInfoMsgBox()
{
	...
	case 2: {
		...
		QString msg(QString("TEAM POKER") + QChar(0xAE) + QString(" - Info Message"));
		inviteOnlyInfoMsgBox->show(INFO_AFTER_JOIN_INVITE_GAME, tr("You have entered a game with type \"invite-only\".\nFeel free to invite other players by right-clicking on their nick in the available players list."), msg, QPixmap(":/gfx/ktip.png"), QDialogButtonBox::Ok, true);
	}
	break;
	default:
		;
...
void gameLobbyDialogImpl::putPlayerOnIgnoreList()
{
	if(myNickListSelectionModel->currentIndex().isValid()) {
		...
		if(!playerIsOnIgnoreList(playerId)) {
			myMessageDialogImpl dialog(myConfig, this);
			if(dialog.exec(IGNORE_PLAYER_QUESTION, tr("You will no longer receive chat messages or game invitations from this user.<br>Do you really want to put player <b>%1</b> on your ignore list?").arg(QString::fromUtf8(mySession->getClientPlayerInfo(playerId).playerName.c_str())), tr("TEAM POKER® - Question"), QPixmap(":/gfx/im-ban-user_64.png"), QDialogButtonBox::Yes|QDialogButtonBox::No, false ) == QDialog::Accepted) {
				list<std::string> playerIgnoreList = myConfig->readConfigStringList("PlayerIgnoreList");
...
void gameLobbyDialogImpl::removePlayerFromIgnoreList()
{
	if(myNickListSelectionModel->currentIndex().isValid()) {
		...
		if(playerIsOnIgnoreList(playerId)) {
			myMessageDialogImpl dialog(myConfig, this);
			if(dialog.exec(UNIGNORE_PLAYER_QUESTION, tr("You will receive chat messages and game invitations from this user again!<br>Do you really want to remove player <b>%1</b> from your ignore list?").arg(QString::fromUtf8(mySession->getClientPlayerInfo(playerId).playerName.c_str())), tr("TEAM POKER® - Question"), QPixmap(":/gfx/dialog_ok_apply.png"), QDialogButtonBox::Yes|QDialogButtonBox::No, false ) == QDialog::Accepted) {
				list<std::string> playerIgnoreList = myConfig->readConfigStringList("PlayerIgnoreList");
...
void gameLobbyDialogImpl::openPlayerStats1()
{
	if(myNickListSelectionModel->currentIndex().isValid()) {
		...
		if(!mySession->getClientPlayerInfo(playerId).isGuest) {
			QUrl url("http://teampoker.com/game/platform/?fn=playerstats&player_nickname="+QUrl::toPercentEncoding(myNickListSelectionModel->currentIndex().data(Qt::DisplayRole).toString()));
...
void gameLobbyDialogImpl::openPlayerStats2()
{
	if(treeWidget_connectedPlayers->currentItem()) {
		...
		if(!mySession->getClientPlayerInfo(playerId).isGuest) {
			QUrl url("http://teampoker.com/game/platform/?fn=playerstats&player_nickname="+QUrl::toPercentEncoding(treeWidget_connectedPlayers->currentItem()->data(0, Qt::DisplayRole).toString()));
			QDesktopServices::openUrl(url);
		}
	}
}
...
void gameLobbyDialogImpl::reportBadGameName()
{
	...
	if (myGameListSelectionModel->hasSelection()) {
		...
		int ret = MyMessageBox::question(this, tr("TEAM POKER® - Question"),
