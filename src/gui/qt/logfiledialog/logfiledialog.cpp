...
void LogFileDialog::refreshLogFileList()
{
	...
	for (i=0; i < dbFilesList.size(); i++) {
		...
		item->setText(0, dbFilesList.at(i).fileName().remove("teampoker-log-"));
#else
...
void LogFileDialog::deleteLogFile()
{
	...
	if(!selectedItemsList.isEmpty() && !( selectedItemsList.size() == 1 && selectedItemsList.front()->data(0, Qt::UserRole+1).toString() == "current" )) {
		...
		QString msg(QString("TEAM POKER") + QChar(0xAE) + QString(" - Delete log files"));
		//int ret = MyMessageBox::warning(this, tr("TEAM POKER\xAE - Delete log files"),
		int ret = MyMessageBox::warning(this, msg,
			...
			for (int i = 0; i < selectedItemsList.size(); ++i) {
				if(selectedItemsList.at(i)->data(0, Qt::UserRole+1).toString() != "current") {
					...
					if(!QFile::remove(selectedItemsList.at(i)->data(0, Qt::UserRole).toString())) {
						MyMessageBox::warning(this, tr("Remove log file"), tr("TEAMPOKER® cannot remove this log file, please verify that you have write access to this file!"), QMessageBox::Close );
					}
				}
			}
			refreshLogFileList();
...
void LogFileDialog::exportLogToHtml()
{
	...
	if(selectedItem) {
		...
		QString msg (QString("Export TEAMPOKER") + QChar(0xAE) + QString(" log file to HTML"));
		QString msg2 (QString("TEAMPOKER") + QChar(0xAE) + QString(" HTML log (*.html)"));
		QString fileName = QFileDialog::getSaveFileName(this, msg,
...
void LogFileDialog::exportLogToTxt()
{
	...
	if(selectedItem) {
		QFileInfo fi(selectedItem->data(0, Qt::UserRole).toString());
		QString msg (QString("Export TEAMPOKER") + QChar(0xAE) + QString(" log file to plain text"));
		QString msg2 (QString("TEAMPOKER") + QChar(0xAE) + QString(" plain text log (*.txt)"));
		QString fileName = QFileDialog::getSaveFileName(this, msg,
...
void LogFileDialog::saveLogFileAs()
{
	...
	if(selectedItem) {
		...
		QString msg (QString("Save TEAM POKER") + QChar(0xAE) + QString(" log file"));
		QString msg2(QString("TEAM POKER") + QChar(0xAE) + QString(" SQL log (*.pdb)"));
		
		QFileInfo fi(selectedItem->data(0, Qt::UserRole).toString());
		//QString fileName = QFileDialog::getSaveFileName(this, tr("Save PokerTH log file"),
		QString fileName = QFileDialog::getSaveFileName(this, msg,
						   QDir::homePath()+"/"+fi.baseName()+".pdb",
						   //tr("TEAMPOKER(R) SQL log (*.pdb)"));
						   msg2);
	...
	if (retStr == LOG_UPLOAD_OK_STR) {
		...
		if (retStr == LOG_UPLOAD_ERROR_STR) {
			...
			case LOG_UPLOAD_ERROR_FILE_HEAD :
				serverMsg += tr("This file is not a valid and current TEAMPOKER® log file.");
				break;
