...
void GameTableStyleReader::readStyleFile(QString file)
{
	...
	if(QFile(file).exists()) {
		...
		currentFileName = QFile(QString::fromUtf8(myConfig->readConfigString("AppDataDir").c_str())+"gfx/gui/table/teampoker/teampoker.xml").fileName();
		fallBack = 1;
	}
...
QString GameTableStyleReader::getFallBackFieldContent(QString field, int type)
{
	...
	QFile myFile(QFile(QString::fromUtf8(myConfig->readConfigString("AppDataDir").c_str())+"gfx/gui/table/teampoker/teampoker.xml").fileName());
	myFile.open(QIODevice::ReadOnly);
		...
		for( ; itemsList; itemsList=itemsList->NextSiblingElement()) {
			...
			if (tmpStr1) {
				...
				if(itemsList->ValueStr() == field.toStdString()) {
					...
					if(type == 1) return QString(QString::fromUtf8(myConfig->readConfigString("AppDataDir").c_str())+"gfx/gui/table/teampoker/"+QString::fromUtf8(tempString1.c_str()));
