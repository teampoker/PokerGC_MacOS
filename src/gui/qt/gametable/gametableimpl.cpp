...
void gameTableImpl::initGui(int speed)
{
	...
	//this->setWindowTitle(QString(titleString + tr("TeamPoker %1").arg(POKERTH_BETA_RELEASE_STRING)));
	this->setWindowTitle(QString(titleString + tr("TEAM POKER® %1").arg(POKERTH_BETA_RELEASE_STRING)));
...
#else
	//this->setWindowTitle(QString(titleString + tr("TeamPoker %1 - The Open-Source Texas Holdem Engine").arg(POKERTH_BETA_RELEASE_STRING)));
	this->setWindowTitle(QString(titleString + tr("TEAM POKER® %1").arg(POKERTH_BETA_RELEASE_STRING)));
...
void gameTableImpl::closeGameTable()
{
	...
	if (myStartWindow->getMyServerGuiInterface() && myStartWindow->getMyServerGuiInterface()->getSession()->isNetworkServerRunning()) {
		...
		MyMessageBox msgBox(QMessageBox::Warning, tr("Closing TEAM POKER® during network game"),
							tr("You are the hosting server. Do you want to close TEAM POKER® anyway?"), QMessageBox::Yes | QMessageBox::No, this);
		...
		if(myUniversalMessageDialog->checkIfMesssageWillBeDisplayed(CLOSE_GAMETABLE_QUESTION ) && (myStartWindow->getSession()->getGameType() == Session::GAME_TYPE_INTERNET || myStartWindow->getSession()->getGameType() == Session::GAME_TYPE_NETWORK ) && this->isVisible()) {
			if (myUniversalMessageDialog->exec(CLOSE_GAMETABLE_QUESTION , tr("Really want to exit?"), tr("TEAM POKER® - Close Table?"), QPixmap(":/gfx/logoChip3D.png"), QDialogButtonBox::Yes|QDialogButtonBox::No, true) == QDialog::Rejected) {
				close = false;
			}
		}
...
void gameTableImpl::leaveCurrentNetworkGame()
{
	...
	if (myStartWindow->getSession()->isNetworkClientRunning()) {
		...
		if(!myUniversalMessageDialog->checkIfMesssageWillBeDisplayed(BACKTO_LOBBY_QUESTION)) {
			...
			if (myUniversalMessageDialog->exec(BACKTO_LOBBY_QUESTION, tr("Attention! Do you really want to leave the current game\nand go back to the lobby?"), tr("TEAM POKER® - Internet Game Message"), QPixmap(":/gfx/logoChip3D.png"), QDialogButtonBox::Yes|QDialogButtonBox::No, true) == QDialog::Accepted) {
				assert(myStartWindow->getSession());
				myStartWindow->getSession()->sendLeaveCurrentGame();
			}
		}
